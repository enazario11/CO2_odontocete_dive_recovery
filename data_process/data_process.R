...
#Beluga data ####
bel_resp <- read.csv(here("data/belugas/DL_Metabolics_Data.csv"))%>% filter(Animal != "Shila"); bel_resp$id <- paste0(bel_resp$Animal, bel_resp$Trial, bel_resp$Date)

bel_vent <- read.csv(here("data/belugas/DL_Vent_Data.csv"))%>% filter(Animal != "Shila"); bel_vent$id <- paste0(bel_vent$Animal, bel_vent$Trial, bel_vent$Date)

bel_blood <- read.csv(here("data/belugas/DL_Blood_Data.csv"))%>% filter(Animal != "Shila"); bel_blood$id <- paste0(bel_blood$Animal, bel_blood$Trial, bel_blood$Date)

bel_flir <- read.csv(here("data/belugas/DL_Vasc_Data.csv"))%>% filter(Animal != "Shila"); bel_flir$id <- paste0(bel_flir$Animal, bel_flir$Trial, bel_flir$Date)
...
all_resp$Animal <- factor(all_resp$Animal, levels = c("Maple", "Qinu", "Nunavik", "Donley", "Rain"))
...
all_vent$Animal <- factor(all_vent$Animal, levels = c("Maple", "Qinu","Nunavik", "Donley", "Rain"))
...
all_blood$Animal <- factor(all_blood$Animal, levels = c("Maple", "Qinu","Nunavik", "Whisper", "Donley", "Rain"))
...
all_vasc$Animal <- factor(all_vasc$Animal, levels = c("Maple", "Qinu","Nunavik", "Whisper", "Donley", "Rain"))
